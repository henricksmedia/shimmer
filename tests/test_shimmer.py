...
import shimmer.bands as bands
import shimmer.mastering as mastering
from shimmer.engine import TransientHoldGate, process
from shimmer.params import MasterParams, Params
...
class TestDSPIntegrity:
    ...
    def test_no_np_roll_in_dsp_alignment(self):
        pkg = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "shimmer")
...
class TestPreview:
    def test_extract_includes_pre_and_post_roll(self):
        from shimmer.server import (_PREVIEW_PREROLL_S, _PREVIEW_POSTROLL_S,
    ...
    def test_preview_duration_exact_and_edges(self):
        from shimmer.server import extract_preview_block, trim_processed_preview
        samples = np.zeros((SR * 10, 2), dtype=np.float32)
...
class TestTrimSilence:
    ...
    def test_trim_bounds_and_padding(self):
        from shimmer.dsp import trim_silence
