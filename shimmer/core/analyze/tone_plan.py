"""Suggested EQ: a per-track corrective EQ plan (the Tone step).

Ported from shimmer/autoeq.py (1.1.1), line for line, so it could be nulled
first (ARCHITECTURE §19.1 item 1). What changed, and only this:

- Its imports come from the engine: the tone target (master.tone.REF_DB),
  the 1/3-octave bands (analyze.track) and the meters.
- The plan's check runs its moves through the engine's EQ (audio.eq), the
  EQ that will apply them. 1.1.1's user EQ also ran bells and shelves both
  ways at half gain, so the EQ itself is the same; the true peak is now read
  per channel at 8x, as everywhere in the engine.

core.tone_plan(source, settings) calls it the way render() will run: judged
after the fixes (the Fixed tones notches) and, with mastering on, after the
mastering tone curve.

Where it sits: the plan is measured at Analyze time and, when applied,
lands in the user EQ stage of the chain (after the fixes and the tone
curve, before mastering's gain), the same place a hand-made EQ goes.
Analysis only measures. Nothing here alters the track; the check works on a
...
How a producer decides, and what this mirrors:
  ...
  2. Judge what the EQ will actually hear. The EQ runs after the fixes, so
     the caller can pass a `cleaner` that runs them on the loudest excerpt;
     the balance is then judged after that change (and after the mastering
     tone curve, when mastering is on) so nothing is corrected twice.
  ...
  5. Genre is a tolerance, not a target. A family sets how far from
     ...
     no genre guessing: Neutral is the default and the family is the
     user's call.
...
the mastering tone target (master.tone.REF_DB).
"""
...
from ..master.tone import REF_DB as _REF_DB
from ..settings import EqBand
from .track import REF_FREQS as _REF_FREQS
from .track import relative_band_levels
...
def _measure(x: np.ndarray, sr: int) -> Dict[str, float]:
    """Integrated loudness and true peak (the louder channel, 8x)."""
    return {"lufs_i": meters.loudness(x, sr), "true_peak_dbtp": meters.true_peak_db(x, sr)}
...
def _apply_moves(x: np.ndarray, sr: int, moves: List[Dict[str, Any]]) -> np.ndarray:
    return user_eq.apply(x, sr, _bands_from_moves(moves)).astype(np.float32)


def moves_to_eq_payload(moves: List[Dict[str, Any]], amount: float = 1.0) -> Dict[str, Any]:
    """The plan as an `eq` payload (the screens' EQ shape)."""
...
def _verify(ex: np.ndarray, sr: int, moves: List[Dict[str, Any]],
    ...
    def run(ms: List[Dict[str, Any]]) -> Dict[str, float]:
        if not ms:
            return before
        return _measure(_apply_moves(ex, sr, ms), sr)
...
def plan_tone(x: np.ndarray, sr: int, family: str = DEFAULT_FAMILY,
    ...
    Args:
        ...
        tone_curve_db: the 29-band correction the mastering stage will
            apply (master.tone.compute_tone_curve). When given, the balance
            is judged AFTER that curve so the two never double up.
        notches: the Fixed tones notch list ({"hz": ...} dicts); lines
            already notched are not planned again.
        cleaner: optional callable that runs the fixes on a clip
            (samples, channels) -> (samples, channels). It is run once on
            the loudest excerpt; the balance is judged after it and the
            plan is checked on the fixed clip, because that is what the EQ
            stage will actually hear.
    """
    ...
    # The loudest excerpt: the check, and the fixes' effect.
    ...
    if cleaner is not None and ex_raw.shape[0] >= sr:
        cleaned = _as_2d(np.asarray(cleaner(ex_raw), dtype=np.float32))
    ...
    # fixes and after the mastering tone curve.
