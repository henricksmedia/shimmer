"""Detect head/tail artifacts that silence trimming cannot catch.

Ported unchanged from shimmer/edges.py (1.1.1), docs/ARCHITECTURE.md §19.1
item 1. The Windows WMI fix it imported first is applied by the shimmer
package itself, before this module loads.
...
(apply_trim below, for an explicit in/out point).
...
def _detect_in_envelope(env_db: np.ndarray, hop: int, x: np.ndarray, sr: int,
    ...
    length_ms = (a_end - a_start) * f2s * 1000.0  # noqa: F841 (kept from 1.1.1)
...
def apply_trim(x: np.ndarray, sr: int,
    ...
    Unlike trim_silence this makes no judgement about content — it cuts
    exactly where asked. Returns (audio, report). Out-of-range or inverted
    points are clamped rather than raising: a bad marker should degrade to
    "no edit", never to a failed export.
    """
    x = as_2d(np.asarray(x, dtype=np.float32))
    n = x.shape[0]
    duration_s = n / sr
