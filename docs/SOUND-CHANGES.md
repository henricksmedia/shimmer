...
| The export and preview run on the new engine | The 1.x chain: tone curve, preset cleaning, de-click, static repair, mastering | Fixed tones (the notch, at full depth as before), the user EQ, then mastering. Until Steps 5 and 6 land there is no tone curve, no preset cleaning and no de-click; every card without a tool says "not built yet" | Live on the rebuild branch |
