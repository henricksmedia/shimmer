...
| Sibilance, Harshness and Low-mid build-up have fixes | Presets aimed at them (`sibilance_rattle`, `harsh_veil`, `muddy_boxy`) | A card turned on runs its fix in the Fixes stage, after the notch: the de-esser for Sibilance (4-10 kHz, up to 7.2 dB at Amount 100 %), the dynamic EQ for Harshness (2-5 kHz, up to 5.1 dB) and for Low-mid build-up (200-500 Hz, up to 4.0 dB). Each Amount's top is set so the fix takes at most 0.10 sones from a clean song ([STEP6-FIXES.md](STEP6-FIXES.md)) | Live on the rebuild branch (2026-09-13), before their blind round |
