...
| Suggested EQ's check on the loudest 20 s | True peak read from a mono mix at 4x | Per channel at 8x. The plans are identical on 12 test tracks; only the check's numbers move. On a cut-only plan for a ringing tone, the peak-to-loudness shift reads 1.07 dB, where 1.1.1 read 0.69, so the line now says the limiter works about 1.1 dB harder |
...
| The export and preview run on the new engine | The 1.x chain: tone curve, preset cleaning, de-click, static repair, mastering | Fixed tones (the notch, at full depth as before), then each Step 6 fix whose card is on (next row), 1.1.1's tone curve (with mastering on), the user EQ, then mastering. No preset cleaning. A card without a passed fix says so ("Not built yet" or "No fix yet") and changes nothing | Live on the rebuild branch |
| Sibilance, Harshness and Low-mid build-up have fixes | Presets aimed at them (`sibilance_rattle`, `harsh_veil`, `muddy_boxy`) | A card turned on runs its fix in the Fixes stage, after the notch: the de-esser for Sibilance (4.5-10 kHz, up to 7.2 dB at Amount 100 %), the dynamic EQ for Harshness (2-5 kHz, up to 5.1 dB) and for Low-mid build-up (200-500 Hz, up to 4.0 dB). Each Amount's top is set so the fix takes at most 0.10 sones from a clean song ([STEP6-FIXES.md](STEP6-FIXES.md)) | Live on the rebuild branch (2026-09-13), before their blind round |
| Shimmer has a fix | Presets aimed at it (`suno_hash`, `broadband_fizz`, `deep_scrub` and others) | A card turned on runs the spectral de-noise in the Fixes stage, after the notch. A trained model sets a gain for each bin in 1.5-16 kHz. It takes at most 0.064 sones from a clean song ([STEP6-FIXES.md](STEP6-FIXES.md)) | Live on the rebuild branch (2026-09-13), to try. It passes on one of its four fault models |
...
| Suggested EQ is judged the way the song renders | Judged after the chosen preset's full cleaning, which the new engine no longer runs | Judged after the fixes the render applies (the Fixed tones notches) and, with mastering on, the tone curve; its moves are checked through the engine's EQ | Live on the rebuild branch |
| The Remix preview matches the export | The loop's own mix, mastered with a level guessed from the original; cleanup and format not heard | At once, the loop's own mix, marked "level approximate". A few seconds later, `render()` on a window of the whole remix, cleanup and format included: the export, within 60 dB (a test). The export reuses that mix | Live on the rebuild branch |
| A FLAC release copy | FLAC only at 24-bit, the song's own rate | A new format, FLAC 16-bit 44.1 kHz with TPDF dither: the same audio as the WAV release copy, about half the size ("Crosscut": 33 MB against 48 MB) | Live on the rebuild branch |
...
| Reference-track matching | None | With a reference track, the tone moves toward it: 50 % of the difference, about an octave of smoothing, at most ±3 dB, level-matched first (MASTERING-SOURCES.md §4). Only when the user picks a reference | Live on the rebuild branch (Master tab, Tone target) |
...
| **2.0.1:** the peak shaper works at 4x, both channels linked | At the base rate, on each channel: at -9 LUFS its fold-back reached -25 to -44 dB against the top end, and -18 to -24 dB under a 9-13 kHz tone ([CHAIN-AUDIT.md](CHAIN-AUDIT.md) §4) | At 4x, with a steep filter on the change only: the fold-back of a 9 kHz tone sits 64 dB under it. Both channels get the same gain. Away from peaks the output is the input, sample for sample | Live |
| **2.0.1:** songs land on their loudness target | One gain from the loudness before mastering; the shaper and limiter then took 0.02-0.32 LU back (-9.09 and -9.10 LUFS on two songs at the -9 target) | The gain is checked once against the finished loudness and corrected: -9.01 on both. A full master takes about 10-18 s, against about 7 | Live |
| **2.0.1:** the tone curve never boosts past the song's cutoff | Set per band and spread between band centres, so a boost reached past the cutoff (+1.1 dB at it on one song); with no cutoff found, +2 dB went onto 16-20 kHz, mostly noise | No bin at or above 90 % of the cutoff is boosted; with none found, nothing above 16 kHz. 1.1.1's 5-12 kHz cap, equal to every boost and so never used, is gone from the built-in target | Live |
| **2.0.1:** the notch leaves held notes alone | On "Alive Again" it cut the note A7 (3514 Hz) by 15.8 dB | A line under 12 kHz, off the generator's 200 Hz grid and within 15 cents of a note, is taken for music. The grid lines found on real songs (14.2-19.2 kHz) are still cut | Live |
| **2.0.1:** 16-bit WAV rounds to the nearest step | libsndfile rounded WAV down (FLAC it rounded): 3 dB more noise and half a step of DC | Rounded to the nearest step for every format: at most half a step, no DC | Live |
| **2.0.1:** Sibilance and Harshness tops measured the way the app runs | Set on the loudest 8 s rendered as a song of its own; planned from the whole song, Harshness took 0.148 sones and Sibilance 0.107 at 100 % | Set again on whole-song plans: Sibilance up to 6.5 dB (was 7.2), Harshness up to 3.3 dB (was 5.1). Low-mid build-up was under the limit and stays | Live |
| **2.3.2:** the tone curve may boost up to +5 dB | Any band was raised by +2 dB at most, so most AI songs stayed dull: the typical library song sits 4.7 dB under the target at 8-16 kHz | Up to +5 dB, with the same target shape, cut limit (-3 dB) and cutoff guard. Blind, level-matched, mastered as Quick would: on five songs whose three versions (+2, +3.5, +5) differed, the author never picked +2, and picked +5 on four. Already-bright songs do not change | Live |
| **2.0.1:** the Vocal grain card | None | The voice de-noise, on the centre of the mix or the vocal alone. Picked by ear; its cost is well over 0.10 sones on clean songs (0.17-2.1 at 50 %), see CHANGELOG | Live, to try |
