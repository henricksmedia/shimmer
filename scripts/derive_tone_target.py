...
def main() -> None:
    ...
    # the captures that actually measured each band, floored so a band they
    # happen to agree on does not become a hair trigger.
    tol = np.array([
        0.5 * float(np.percentile(cap[usable[:, j], j], 84)
                    - np.percentile(cap[usable[:, j], j], 16))
        if ok_band[j] else np.nan for j in range(len(F))])
    tol = np.maximum(tol, 1.5)
    # A band nobody measured must not end up with the tightest tolerance in
    # the array — that is the opposite of what not knowing means. Give it the
    # widest of its neighbours within an octave, and never less than 6 dB.
    lf = np.log2(F)
    for j in range(len(F)):
        if ok_band[j]:
            continue
        near = ok_band & (np.abs(lf - lf[j]) <= 1.0)
        tol[j] = max(6.0, float(np.max(tol[near])) if near.any() else 6.0)
